...
docker cp ${VOLUME}/Backup-Data/prod-export.sql  isle-mysql-ld:/prod-export.sql
echo ${GOOD}
...
echo "  mysql -u root -p digital_grinnell < prod-export.sql > /tmp/mysqldump.log 2>&1; mysql -u root -p digital_grinnell -e 'show tables;'; exit; "
echo ""
echo ${ATTENTION}
